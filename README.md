...
Each config entry creates one diagnostic entity, **Quarter Hour Power Status**. Its state reports the current interval-processing status, for example `ok`, `baseline_missing`, `source_changed`, `invalid_grid_source`, `invalid_house_source`, `negative_grid_delta`, `negative_house_delta`, `interval_too_short` or `interval_too_long`.

The sensor exposes compact diagnostic attributes including the last valid interval, interval timestamps, grid delta/power, configured source, Recorder recovery status, discarded-interval count and last discard reason. If a house source is configured, house delta/power, reduction and house source are included as additional attributes. In **Grid-only mode, all house-related attributes are omitted entirely** rather than being published as empty or `null` values.
...
☀️ PV production, 🔋 battery power and ↗️ grid export are not required. When a house source is configured, the difference between house consumption and grid import is treated as **local supply / grid-import reduction**. A battery is therefore intentionally not analysed separately.
...
## 🏷️ Prefix and multiple instances

An optional entity-name prefix can be configured. Multiple config entries are supported, including multiple instances that use the **same grid/house source combination**. This is useful for parallel setups, testing, or different presentation prefixes, for example:
...
- ✅ The Home Assistant integration is declared as a `service`.
- 🔁 Multiple config entries are allowed; the source combination is intentionally **not** used as a config-entry unique ID.
- 🏡 The house energy source is optional. Without it, house, grid-reduction, and percentage entities are not created.
- 🧹 Release archives exclude Python cache files such as `__pycache__/` and `*.pyc`.
...
- ⏱️ the next quarter-hour boundary establishes a fresh baseline,
